...
![](https://gitee.com/oneNorth7/pics/raw/master/picgo/暂不支持该格式在线预览或播放.jpg)
...
* **多文件分享页面**
1. 脚本加载成功后，文件大小前的**小下载按钮**的提示文字会由`下载`变为`直接下载`，文件大小超过1G的文件会添加小下载按钮，点此小按钮可下载单个文件，当发现提示文字不是`直接下载`时请**刷新页面**重新加载
...
3. 支持**图标**模式下选中文件后**直接下载**，由于**图标**模式下需要选中文件后才显示出**全选**按钮，**不推荐使用此模式**
