...
## 紧急通知：天翼云官网升级改版，雷利子暂时无法封印解除

可关注个人公众号后点击下方菜单 `油猴脚本` -> `雷利子` 获取临时方案，本人会抓紧时间分析寻找突破口，感谢各位的使用和支持！！！
...
* 启用“雷利子”脚本后，未登录帐号会提示”**请先登录**，必须登录才能突破下载限制“，点击`OK`后会弹出登录界面
...
* 点选文件后若**很久**都没有出现绿色的**直接下载**按钮，可以**尝试点选其他文件或取消或再选中或刷新页面**

* 目录**切换过快**时可能导致**逐个下载**按钮不显示或不消失，可以**尝试刷新页面**（**当页面有多个文件夹而逐个下载按钮还显示的时候请注意，请立即刷新页面，千万别点逐个下载！！！不然点击后弹出的下载文件名称都是`【打包下载】天翼云盘.zip`，无法区分**）
...
* **打包下载**返回的链接的压缩包名固定为`【打包下载】天翼云盘.zip`，并且该链接**不支持**IDM多线程和续传下载，速度也**降下来**了
* 有**大批量**下载文件或文件夹需求的还是建议选择**官方客户端**下载
